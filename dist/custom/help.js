...
export default class CustomHelp extends Help {
    ...
    formatCommands(commands) {
        ...
        for (const command of commands) {
            ...
            const args = Object.keys(command.args).length > 0
    ...
    formatTopic(topic) {
        ...
        if (topic.description)
            out += chalk.dim(topic.description) + "\n";
        out += chalk.bold("Usage: modus " + topic.name) + " " + chalk.bold.blue("[command]") + "\n";
        return out;
    }
    formatTopics(topics) {
        let out = "";
        if (topics.find((v) => !v.hidden))
            out += chalk.bold("Tools:") + "\n";
        else
            return out;
    ...
    async showRootHelp() {
        ...
        for (const command of rootCommands) {
            if (command.id.length > this.pre_pad)
                this.pre_pad = command.id.length;
            const args = Object.keys(command.args).length > 0
                ? Object.entries(command.args)
                    .map((v) => {
                    if (!v[1].hidden && v[1].required) {
                    ...
                    .join(" ")
                : "";
            if (args.length > this.post_pad)
                this.post_pad = args.length;
        }
    ...
    async showTopicHelp(topic) {
        ...
        for (const command of commands) {
            if (command.id.split(":")[1].length > this.pre_pad)
                this.pre_pad = command.id.split(":")[1].length;
            const args = Object.keys(command.args).length > 0
                ? Object.entries(command.args)
                    .map((v) => {
                    if (!v[1].hidden && v[1].required) {
    ...
    async showCommandHelp(command) {
        ...
        if (command.description)
            this.log(chalk.dim(command.description));
