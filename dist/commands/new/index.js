...
export default class NewCommand extends Command {
    ...
    async run() {
        ...
        if (!flags.force && !(await this.confirmAction(rl, "[3/4] Continue? [y/n]")))
            clearLine(), clearLine(), process.exit(0);
        this.installProject(name, dir, sdk, flags.force, rl);
    }
    async promptProjectName(rl) {
    ...
    async promptSdkSelection(rl) {
        ...
        for (const _ of Object.values(SDK))
            clearLine();
        if (!sdk)
            process.exit(1);
    ...
    async confirmAction(rl, message) {
        this.log(message);
