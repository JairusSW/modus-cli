...
export default class SDKInstallCommand extends Command {
    ...
    async run() {
        ...
        if (ctx.args.version)
            await this.installVersion(ctx);
        else if (ctx.flags.branch)
            await this.installCommit(ctx);
        else if (ctx.flags.link)
            await this.linkDir(ctx);
    }
    async installVersion(ctx) {
...
async function fetchCommit(repo, commitSha) {
...
function linkDirectories(srcDir, destDir) {
